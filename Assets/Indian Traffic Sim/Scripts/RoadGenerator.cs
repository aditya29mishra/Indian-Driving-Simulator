...
public class RoadGenerator : MonoBehaviour
{
    ...
    public void GenerateRoad()
    {
        ...
        if (road == null)
        {
            Debug.LogError("TrafficRoad component missing!");
            return;
        }

        // Unique ID (no manual input)
        roadId = $"R{GetInstanceID()}";
        ...
        for (int i = 0; i < totalLanes; i++)
        {
            ...
            GameObject laneObj = new GameObject($"Lane_{roadId}_{dir}_{groupIndex}");
            ...
            GameObject pathObj = new GameObject($"Path_{roadId}_{dir}_{groupIndex}");
            ...
            for (int w = 0; w <= waypointCount; w++)
            {
                ...
                GameObject wp = new GameObject($"WP_{roadId}_{dir}_{groupIndex}_{w}");
