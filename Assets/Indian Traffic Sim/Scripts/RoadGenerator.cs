...
public class RoadGenerator : MonoBehaviour
{
    ...
    public void GenerateRoad()
    {
        ...
        for (int i = 0; i < totalLanes; i++)
        {
            ...
            {
                groupIndex = i;
                groupSize = forwardLanes;
            }
            else
            {
                groupIndex = i - forwardLanes;
                groupSize = backwardLanes;
            }

            // Center lanes inside each direction group
            float offsetFromCenter = ((groupIndex - (groupSize - 1) * 0.5f) * laneWidth);

            // Shift forward and backward groups to opposite sides of road
            ...
            // Reverse direction for backward lanes
            if (!isForward)
            {
                Vector3 temp = laneStart;
                laneStart = laneEnd;
                laneEnd = temp;
            }
