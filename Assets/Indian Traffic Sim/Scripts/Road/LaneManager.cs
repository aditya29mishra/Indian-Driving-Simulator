...
public class LaneManager : MonoBehaviour
{
    ...
    void UpdateLaneOrdering()
    {
        foreach (var kvp in laneVehicles)
        {
            ...
            // Sort front-to-back by distance travelled on the path
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                {
                    // Green-light restart: only suppress the front vehicle's leader
                    // so the queue clears with a natural ripple — each car follows its
                    // predecessor via IDM instead of all launching simultaneously.
                    // QueueRippleScenario.IsFrontOfQueue is true when no vehicle ahead
                    // is also in WaitingRed state — i.e. this vehicle IS the queue front.
